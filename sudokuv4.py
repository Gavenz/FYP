...
class SudokuTeachViz:
    def __init__(self, puzzle):
        ...
        self.pc_sets = {
            ...
            'For': [
                "solve(board):",
                "  empty_cells → list_empty_cells(board)        # fixed visiting order",
                "  stack → []                                   # frames: (row, col, allowed, next_idx)",
                "  cell_idx → 0                                  # which empty cell we’re on",
                "  while cell_idx < length(empty_cells):",
                "    (row, col) → empty_cells[cell_idx]",
                "    if stack is empty or top(stack).(row,col) ≠ (row,col):",
                "      allowed → candidates_for_cell(board, row, col); push(stack, (row, col, allowed, 0))",
                "    frame → top(stack); placed → False",
                "    while frame.next_idx < length(frame.allowed):",
                "      digit → frame.allowed[frame.next_idx]; frame.next_idx → frame.next_idx + 1",
                "      if is_valid(board, row, col, digit):",
                "        board[row][col] → digit; placed → True; break      # place & bookmark next_idx",
                "    if placed: cell_idx → cell_idx + 1; continue           # advance to next cell",
                "    pop(stack); board[row][col] → 0; cell_idx → cell_idx - 1  # backtrack",
        ...
        # --- For-mode teaching state (UI-only; engine unchanged)
        self.saved_next_index = {}   # (r,c) -> next j to try
        self.cell_cands = {}         # (r,c) -> candidates list from 'candidates' event
        self.last_try_index = {}     # (r,c) -> j used on the last valid try
        self.for_i = 0               # conceptual i (index into empty_cells)
    ...
    def reset_visual_state_only(self):
        ...
        # reset For-mode teaching state
        self.saved_next_index.clear()
        self.cell_cands.clear()
    ...
    def describe_event(self, event, replay_info=None):
        ...
        if et == 'base_case':
            ...
            if self.mode != 'For':
                ...
                # For-mode bookkeeping + contextual messages
                self.cell_cands[(r,c)] = list(cands)
                start = self.saved_next_index.get((r,c), 0)
                header = f"[For] i={self.for_i} → cell r{r+1},c{c+1}"
                info   = f"Candidates: {cands}\nResume from start_index={start}"
                self.set_status(header, info)
        elif et == 'try':
            ...
            if self.mode != 'For':
                ...
                cands = self.cell_cands.get((r,c), [])
                j = cands.index(d) if d in cands else None
                result = "valid ✓" if event['valid'] else "invalid ✗"
                line1 = f"Try {d} at r{r+1},c{c+1}" + (f" (j={j})" if j is not None else "") + f" → {result}"
                if event['valid']:
                    self.set_status(line1, "")
                    if j is not None:
                        self.last_try_index[(r,c)] = j
                else:
                    reason = event.get('reason')
                    if reason:
                        cells = reason['conflicts']
                        where = ", ".join([f"(r{ri+1},c{cj+1})" for (ri, cj) in cells])
                        self.set_status(line1, f"Violates {reason['rule']}: {where}.")
                    else:
                        self.set_status(line1, "")
        elif et == 'place':
            r,c = event['cell']; d = event['digit']
            if self.mode != 'For':
                ...
                j = self.last_try_index.get((r,c), None)
                nxt = (j + 1) if j is not None else 0
                self.saved_next_index[(r,c)] = nxt
                line1 = f"Place {d} at r{r+1},c{c+1}"
                line2 = f"saved_next_index[(r{r+1},c{c+1})] ← {nxt}"
                line3 = f"Advance: i ← {self.for_i} → {self.for_i + 1}"
                self.for_i += 1
                self.set_status(line1, line2 + "\n" + line3)
            ...
            if self.mode != 'For':
                ...
                self.set_status("Return step",
                                "→ success (keep advancing i)" if ok else "→ dead end below (will backtrack)")
        elif et == 'backtrack':
            r,c = event['cell']
            if self.mode != 'For':
                ...
                line1 = f"No valid candidate at r{r+1},c{c+1}"
                line2 = "Reset cell to 0"
                line3 = f"Backtrack: i ← {self.for_i} → {max(0, self.for_i - 1)}"
                self.for_i = max(0, self.for_i - 1)
                self.set_status(line1, line2 + "\n" + line3)
        elif et == 'dead_end':
            r,c = event['cell']; cands = event['cands']
            if self.mode != 'For':
                ...
                start = self.saved_next_index.get((r,c), 0)
                self.set_status(f"Dead end at r{r+1},c{c+1}",
                                f"All tried from start_index={start} → backtrack.")
