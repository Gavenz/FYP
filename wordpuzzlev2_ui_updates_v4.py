...
def _sift_down_minheap(H, i, n, steps):
    ...
    while True:
        ...
        steps.append((f"compare: i={i} ({H[i][1]}) vs child={smallest} ({H[smallest][1]})",
                      list(H), i, smallest))
        ...
        if H[smallest][0] < H[i][0]:
            ...
            steps.append((f"sift-down swap: i={i} <-> {smallest}", list(H), smallest, None))
            i = smallest
        else:
            # optional: show that no swap is needed after comparison
            steps.append((f"stop: MaxHeap property holds at i={i}", list(H), i, smallest))
            break
...
def push_with_snapshots(heap_arr, item):
    ...
    steps.append(("MaxHeap before insert", list(H), None, None))

    H.append(item)
    i = len(H) - 1
    steps.append((f"append new item at index {i} → {item[1]} (len={-item[0]})", list(H), i, None))

    while i > 0:
        p = (i - 1) // 2
        steps.append((
            f"compare: i={i} ({H[i][1]}, key={H[i][0]}) vs parent={p} ({H[p][1]}, key={H[p][0]})",
            list(H), i, p
        ))

        if H[i][0] < H[p][0]:
            H[i], H[p] = H[p], H[i]
            steps.append((f"sift-up swap: i={i} <-> parent={p}", list(H), p, None))
            i = p
        else:
            steps.append(("stop: MaxHeap property holds (no swap needed)", list(H), i, p))
            break

    steps.append(("insertion complete (MaxHeap property satisfied)", list(H), i, None))
    return steps, H
...
def heapify_with_snapshots(arr):
    ...
    """
    H = list(arr)
    steps = [("start (unsorted array)", list(H), None, None)]
    n = len(H)
    for i in range(n//2 - 1, -1, -1):
        steps.append((f"sift-down from i={i}", list(H), i, None))
        _sift_down_minheap(H, i, n, steps)
    steps.append(("heapify complete (maxheap property satisfied)", list(H), None, None))
    return H, steps
...
def pop_with_snapshots(heap_arr):
    ...
    steps.append(("MaxHeap start", list(H), 0, None))

    last = len(H) - 1
    H[0], H[last] = H[last], H[0]
    steps.append((f"swap root (0) with last ({last})", list(H), 0, last))

    popped = H.pop()
    steps.append((f"remove last → popped {popped[1]} (len={-popped[0]})", list(H), 0, None))
    ...
    while True:
        ...
        r = 2*i + 2
        if l >= n:
            steps.append((f"stop: no children at i={i}", list(H), i, None))
            break
        ...
        if H[smallest][0] < H[i][0]:
            H[i], H[smallest] = H[smallest], H[i]
            steps.append((f"sift-down swap: i={i} <-> child={smallest}", list(H), smallest, None))
            i = smallest
        else:
            steps.append((f"stop: MaxHeap property holds at i={i}", list(H), i, smallest))
            break
    ...
    def step(self):
        ...
        if self.phase == "show_insert":
            if not self.heap_insert_demo:
                ...
                return {
                    ...
                    "grid": self.grid,
                    "remaining": self.remaining,
                    "pq_msg": "Insertion complete — now begin deletion (pop longest)",
                    "heap_msg": "",
        ...
        if self.phase == "pop_word":
            if not self.heap_arr:
                ...
                return {
                    ...
                    "grid": self.grid,
                    "remaining": self.remaining,
                    "pq_msg": "MaxHeap empty → fill random letters",
                    "heap_msg": "",
                    "heap_view": "tree",
                    "heap_array": [],
                    "heap_array_view": [],
        ...
        if self.phase == "choose_placement":
            ...
            if best is None:
                ...
                return {
                    ...
                    "remaining": self.remaining,
                    "highlight": None,
                    "pq_msg": f"Step 3: evaluate placements → none valid for '{w}'",
                    "heap_msg": "",
                    "heap_array": list(self.heap_arr),
                }
            ...
            return {
                ...
                "remaining": self.remaining,
                "highlight": cells,
                "pq_msg": f"Step 3: evaluate placements → choose max overlap = {ov}",
                "heap_msg": "",
                "heap_array": list(self.heap_arr),
            }
        ...
        if self.phase == "fill":
            ...
            return {
                ...
                "grid": self.grid,
                "remaining": self.remaining,
                "pq_msg": "Step 5: filled remaining cells with random letters",
                "heap_msg": "",
                "heap_array": list(self.heap_arr),
            }
    ...
    def draw_heap_tree(self, remaining_words, footnote="", heap_array=None, active_index = None, secondary_index = None):
        ...
        if not arr:
            ...
            for i in range(1, n):
                ...
                ax.plot([xs[p], xs[i]], [ys[p], ys[i]], color="0.7", zorder=1)
            for i, (score, w) in enumerate(disp):
                ...
                # Comparison phase (decision): shade the compared-against node(s) grey.
                # If you prefer shading BOTH nodes, replace the condition with: (i in compare_set)
                if in_compare and (secondary_index is not None) and (i in compare_set) and (i != active_index):
                    # slightly darker than white so it's visible on screenshots
                    fc = "0.85"
                    ec = "0.45"
                    lw = 2.2
        ...
        if self.mode == "generate":
            ...
            if hv == "array":
                ...
                viz.draw_heap_tree(
                    gen.remaining,
                    footnote=snap.get("heap_msg",""),
                    # Use the snapshot's heap array so the tree view mirrors the PQ array + indices.
                    heap_array=snap.get("heap_array_view", list(gen.heap_arr)),
