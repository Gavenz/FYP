...
def push_with_snapshots(heap_arr, item):
    ...
    steps.append(("heap before insert", list(H), None, None))
    ...
    while i > 0:
        ...
        if H[i][0] < H[p][0]:
            ...
            steps.append(("stop: heap property holds (no swap needed)", list(H), i, p))
            break

    steps.append(("insertion complete (heap property satisfied)", list(H), i, None))
    return steps, H
...
def heapify_with_snapshots(arr):
    ...
    steps.append(("heapify complete (heap property satisfied)", list(H), None, None))
    return H, steps
...
def pop_with_snapshots(heap_arr):
    ...
    steps = []
    if not H:
        return None, [("heap empty", [], None, None)], []

    steps.append(("heap start", list(H), 0, None))
    ...
    while True:
        ...
        if H[smallest][0] < H[i][0]:
            ...
            steps.append((f"stop: heap property holds at i={i}", list(H), i, smallest))
            break

    steps.append(("heap restored", list(H), None, None))
...
def run():
    ...
    while True:
        ...
        if ctl.restart:
            ...
            prev_msg = ""
            cur_msg = "Click Step to begin heapify"
            viz.draw_pq_array_in_log(initial_arr, title="PQ array", footnote=cur_msg, prev_footnote=prev_msg)
